...
src/test/simple-master-data.test.ts` instead — this doc is for getting an
...
```sh
cd server
bun run seed/dev-data-academic.ts
bun run seed/dev-data-academic.ts --clean # clear all data after running this walkthrough
...
rather than assuming the Employee seed has already run, so either script works alone,
in any order, against a fresh DB.

It prints a `--- Copy-paste to set up your shell ---` block. Copy that
...
Two employees are seeded specifically to demonstrate the homeroom-teacher
eligibility rule:
- **Teacher** (`$TEACHER_EMPLOYEE_ID`) — job level `is_teaching_role: true`,
  can be assigned as a homeroom teacher.
- **Staff** (`$STAFF_EMPLOYEE_ID`) — job level `is_teaching_role: false`,
  gets rejected if you try.

`$GRADE_ID` is a lookup of the real, permanently-seeded "Grade 1" row (see
§3) — Grade has no CRUD API, so there's nothing to create there.
...
homeroom teacher), so it has its own dedicated service — but the same
shape of endpoints.
...
> Employee soft-delete call in `docs/employee-walkthrough.md` §5 — Hono's
> `csrf()` middleware treats a missing `Content-Type` as `text/plain` and
> blocks it unless the request looks same-origin. Always send
...
a Job Level with employees at that level) is rejected:
...
enrollment is rejected the same way as the master-data delete-guards above.

## 3. Grade

No CRUD here — Grade is fixed reference data (Kindergarten Pre-K/K1/K2,
Grade 1–9), seeded once via migration, same as Unit/Job Position/Job Level
used to be conceptually but isn't in practice since those genuinely change
over time. Read-only:
...
A class name only needs to be unique **within its academic year** — the
same name is fine in a different year (this is exactly what happens across
...
`homeroom_teacher_id` must be a currently-**active** employee whose job
level is **teaching-eligible** (`is_teaching_role: true`):
...
Deleting a class still referenced by a current student or an enrollment is
rejected the same way as the other delete-guards above.

## 5. Homeroom teacher assignment history

`Class.homeroom_teacher_id` only holds the **current** homeroom teacher.
Every assignment — past and present — is kept in a separate history table,
so reassigning a class never loses the record of who taught it before:

```sh
# reassign — closes the current assignment, opens a new one
curl -s -X PATCH -H "Content-Type: application/json" \
  -H "Cookie: access_token=$ACADEMIC_ADMIN_TOKEN" \
...
- **Every rule above, plus every edge case** (invalid enums, missing
  fields, race-condition safety nets, RBAC per role, delete-guards for
  every reference type): `bun test src/test/academic-year.test.ts
...
- **Who changed what, when**: `AuditLog` rows written on every
  ...
  Studio (`bunx prisma studio`) or directly against the `audit_logs` table.
  Academic Year, Class, and Grade/Unit/Job Position/Job Level mutations use
  `CREATE_ACADEMIC_YEAR`/`UPDATE_ACADEMIC_YEAR`/`DELETE_ACADEMIC_YEAR`,
  `CREATE_CLASS`/`UPDATE_CLASS`/`DELETE_CLASS`, and the generic
