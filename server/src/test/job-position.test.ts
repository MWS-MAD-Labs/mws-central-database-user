...
describe("POST /api/admin/job-positions", () => {
  beforeEach(async () => {
  ...
  it("should successfully create a job position when requested by SUPER_ADMIN", async () => {
    const { accessToken } = await AdminUserTest.createSuperAdmin();

    const response = await TestRequest.post(
      "/api/admin/job-positions",
      { name: "TEST_Art Teacher", is_teaching_position: true },
      accessToken,
    );
    ...
    expect(body.data.name).toBe("TEST_Art Teacher");
    expect(body.data.is_teaching_position).toBe(true);
    ...
    expect(auditLog.entity_type).toBe("MasterJobPosition");
  ...
  it("should reject if no access token provided", async () => {
    const response = await TestRequest.post("/api/admin/job-positions", {
      name: "TEST_NoAuth",
    });
    const body = await response.json();
    logger.debug(body);
...
describe("PATCH /api/admin/job-positions/:id", () => {
  ...
  it("should successfully update a job position's is_teaching_position flag", async () => {
    const { accessToken } = await AdminUserTest.createSuperAdmin();
    const jobPosition = await prismaClient.masterJobPosition.create({
      data: { name: "TEST_Original", is_teaching_position: false },
    });
    ...
    const auditLog = await prismaClient.auditLog.findFirstOrThrow({
      where: { entity_id: jobPosition.id },
    });
    logger.debug(auditLog);

    expect(auditLog.action).toBe(AuditAction.UPDATE_MASTER_DATA);
    expect(auditLog.admin_id).toBe(admin.id);
    const oldValues = auditLog.old_values as { is_teaching_position?: boolean };
    const newValues = auditLog.new_values as { is_teaching_position?: boolean };
    expect(oldValues?.is_teaching_position).toBe(false);
    expect(newValues?.is_teaching_position).toBe(true);
  });
  ...
  it("should reject renaming to an already-used name", async () => {
    const { accessToken } = await AdminUserTest.createSuperAdmin();
    const taken = await prismaClient.masterJobPosition.create({
      data: { name: "TEST_Taken" },
    });
    const other = await prismaClient.masterJobPosition.create({
      data: { name: "TEST_ToRename" },
    });

    const response = await TestRequest.patch(
      `/api/admin/job-positions/${other.id}`,
      { name: taken.name },
      accessToken,
    );
    const body = await response.json();
  ...
  it("should allow re-saving with the same name (no-op rename)", async () => {
    ...
    expect(response.status).toBe(200);
    expect(body.data.name).toBe(jobPosition.name);
  });
  ...
  it("should reject if the job position does not exist", async () => {
    ...
    const response = await TestRequest.patch(
      "/api/admin/job-positions/invalid-cuid-123",
      { name: "TEST_Whatever" },
      accessToken,
    );
    const body = await response.json();
...
describe("GET /api/admin/job-positions/:id", () => {
  beforeEach(async () => {
    await AuditLogTest.delete();
    await AdminUserTest.delete();
    await MasterDataTest.delete();
...
describe("GET /api/admin/job-positions", () => {
  beforeEach(async () => {
    await AuditLogTest.delete();
    await AdminUserTest.delete();
    await MasterDataTest.delete();
  ...
  it("should list and paginate", async () => {
    const { accessToken } = await AdminUserTest.createSuperAdmin();
    await prismaClient.masterJobPosition.create({
      data: { name: "TEST_CRUD_A" },
    });
    await prismaClient.masterJobPosition.create({
      data: { name: "TEST_CRUD_B" },
    });
    await prismaClient.masterJobPosition.create({
      data: { name: "TEST_CRUD_C" },
    });

    const response = await TestRequest.get(
      "/api/admin/job-positions?size=2&page=1&search=TEST_CRUD",
      accessToken,
    );
    const body = await response.json();
    logger.debug(body);
  ...
  it("should search by name", async () => {
    ...
    await prismaClient.masterJobPosition.create({
      data: { name: "TEST_Fedora" },
    });

    const response = await TestRequest.get(
      "/api/admin/job-positions?search=sombrero",
      accessToken,
    );
    const body = await response.json();
    logger.debug(body);
  ...
  it("should sort by name descending when requested", async () => {
    ...
    const response = await TestRequest.get(
      "/api/admin/job-positions?search=TEST_CRUD&sort_by=name&sort_order=desc",
      accessToken,
    );
    const body = await response.json();
    logger.debug(body);
  ...
  it("should reject an invalid sort_by field", async () => {
    ...
    const response = await TestRequest.get(
      "/api/admin/job-positions?sort_by=not_a_real_field",
      accessToken,
    );
    const body = await response.json();
    logger.debug(body);
  ...
  it("should reject a non-numeric page", async () => {
    ...
    const response = await TestRequest.get(
      "/api/admin/job-positions?page=abc",
      accessToken,
    );
    const body = await response.json();
    logger.debug(body);
  ...
  it("should reject a size greater than the maximum allowed (100)", async () => {
    ...
    const response = await TestRequest.get(
      "/api/admin/job-positions?size=101",
      accessToken,
    );
    const body = await response.json();
    logger.debug(body);
  ...
  it("should reject if no access token provided", async () => {
    const response = await TestRequest.get("/api/admin/job-positions");
    const body = await response.json();
    logger.debug(body);
...
describe("DELETE /api/admin/job-positions/:id", () => {
  let masterData: {
    unit: MasterUnit;
  ...
  it("should delete a job position not referenced by anything", async () => {
    const { accessToken } = await AdminUserTest.createSuperAdmin();
    const jobPosition = await prismaClient.masterJobPosition.create({
    ...
    const stillThere = await prismaClient.masterJobPosition.findUnique({
